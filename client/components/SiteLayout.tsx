    ...
    <div className="min-h-screen overflow-hidden bg-[#f7f7f2] text-[#13201e]">
      <header className="relative z-30 border-b border-[#13201e]/10 bg-[#f7f7f2]/90 backdrop-blur-md">
        <div className="page-shell flex h-[76px] items-center justify-between">
          ...
          <a href="https://courses.schoolofdevops.com/" target="_blank" rel="noopener noreferrer" className="hidden rounded-full border border-[#13201e]/15 px-4 py-2 text-xs font-bold transition hover:border-[#13201e] md:block">Legacy learner login</a>
        ...
        {open && <div className="page-shell border-t border-[#13201e]/10 py-5 md:hidden"><nav className="flex flex-col gap-4 text-sm font-bold">{links.map((link) => <NavLink onClick={() => setOpen(false)} key={link.to} to={link.to}>{link.label}</NavLink>)}<a href="https://courses.schoolofdevops.com/" target="_blank" rel="noopener noreferrer" onClick={() => setOpen(false)}>Legacy learner login</a></nav></div>}
      </header>
      <Outlet />
