    ...
    <div className="min-h-screen overflow-hidden bg-[#f6f3f8] text-[#1e1a2e]">
      <header className="relative z-30 border-b border-[#1e1a2e]/10 bg-[#f6f3f8]/90 backdrop-blur-md">
        <div className="page-shell flex h-[76px] items-center justify-between">
          ...
          <a href="https://courses.schoolofdevops.com/" target="_blank" rel="noopener noreferrer" className="hidden rounded-full border border-[#1e1a2e]/15 px-4 py-2 text-xs font-bold transition hover:border-[#1e1a2e] md:block">Login</a>
        ...
        {open && <div className="page-shell border-t border-[#1e1a2e]/10 py-5 md:hidden"><nav className="flex flex-col gap-4 text-sm font-bold">{links.map((link) => <NavLink onClick={() => setOpen(false)} key={link.to} to={link.to}>{link.label}</NavLink>)}<a href="https://courses.schoolofdevops.com/" target="_blank" rel="noopener noreferrer" onClick={() => setOpen(false)}>Login</a></nav></div>}
      </header>
      <Outlet />
