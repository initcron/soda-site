    ...
    <div className="min-h-screen overflow-hidden bg-[#ffffff] text-[#506580]">
      <header className="relative z-30 border-b border-[#506580]/10 bg-[#ffffff]/90 backdrop-blur-md">
        <div className="page-shell flex h-[76px] items-center justify-between">
          <Link to="/" className="flex items-center gap-3" onClick={() => setOpen(false)}>
            <span className="flex h-10 w-10 items-center justify-center rounded-full bg-[#D4A08A] font-display text-lg font-black tracking-[-0.1em] text-white">S</span>
          ...
          <nav className="hidden items-center gap-7 text-[13px] font-semibold md:flex">
            {links.map((link) => <NavLink key={link.to} to={link.to} className={({ isActive }) => `transition-colors hover:text-[#B07A64] ${isActive ? "text-[#B07A64]" : ""}`}>{link.label}</NavLink>)}
          </nav>
      ...
      <footer className="border-t border-[#506580]/15 bg-[#506580] text-[#ffffff]">
        <div className="page-shell grid gap-10 py-14 md:grid-cols-[1.3fr_2fr]">
          <div><div className="flex items-center gap-3"><span className="flex h-9 w-9 items-center justify-center rounded-full bg-[#D4A08A] font-display text-base font-black text-white">S</span><span className="font-display text-xl font-extrabold tracking-[-0.06em]">School of DevOps.</span></div><p className="mt-5 max-w-xs text-sm leading-6 text-[#b8bac0]">Production-grade learning for the systems that run modern AI.</p></div>
          <div className="grid grid-cols-2 gap-8 text-sm sm:grid-cols-3"><div className="space-y-3"><p className="text-xs font-bold uppercase tracking-[0.14em] text-[#8a8c94]">Explore</p><Link to="/courses" className="block hover:text-[#D4A08A]">Courses</Link><Link to="/learning-paths" className="block hover:text-[#D4A08A]">Learning paths</Link><Link to="/roadmaps" className="block hover:text-[#D4A08A]">Roadmaps</Link></div><div className="space-y-3"><p className="text-xs font-bold uppercase tracking-[0.14em] text-[#8a8c94]">Company</p><Link to="/about" className="block hover:text-[#D4A08A]">About</Link><a href="https://runbooks.schoolofdevops.com/" target="_blank" rel="noopener noreferrer" className="block hover:text-[#D4A08A]">Newsletter</a><a href="https://courses.schoolofdevops.com/dashboard/" target="_blank" rel="noopener noreferrer" className="block hover:text-[#D4A08A]">Login</a></div><div className="space-y-3"><p className="text-xs font-bold uppercase tracking-[0.14em] text-[#8a8c94]">Legal</p><Link to="/legal" className="block hover:text-[#D4A08A]">Privacy & disclosure</Link></div></div>
        </div>
